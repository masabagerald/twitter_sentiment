...
;;loading json file ,json file is passed as an argument

(define (load-JSON file-path)
(string->jsexpr
                (with-input-from-file file-path (λ () (json-lines->json-array))))
 )

;(define (load-JSON file-path) (string->jsexpr
   ;             (with-input-from-file file-path (λ () (json-lines->json-array)))))

;;another option to load json file
(define (input_tweets file_path)(port->string(open-input-file file_path)))


(define (remove-urls json-string)
  (regexp-replace* #px"http[s]?://[^ ]+\\b" json-string " "))

;a procedure to normalizes json string
(define (json-cleaner json-string)
  (string-append "\"" (regexp-replace* "\"" json-string "\"\"") "\""))
...
;converting timestamp commonly used in api to readable date time format
(define (convert-timestamp string)
  (string->date string "~a ~b ~d ~H:~M:~S ~z ~Y"))
...
(define (ugandan-time list)
  (map list
       ($ list 0)
       (append (drop ($ list 1) 4) (take ($ list 1) 4))))
...
(define extract
  (let ([tmp (map (λ (x) (list (hash-ref x `full_text)(hash-ref x `favorite_count)
                               (hash-ref x `created_at)(hash-ref x `source))) load-JSON)])
...
;;enerating a bar ggraph
(define (bar-graph width x-label y-label title)
...
(define polarity_graph (list->sentiment words #:lexicon 'bing))
